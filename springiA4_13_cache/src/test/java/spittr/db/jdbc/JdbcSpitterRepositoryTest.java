...
public class JdbcSpitterRepositoryTest {
    ...
    public void testCache() {
        System.out.println(spitterRepository.count());//清除缓存
        spitterRepository.findAll();//进入方法，加缓存
        spitterRepository.findAll();//从缓存中取，不进入方法
        Spitter spitter = new Spitter(null, "newbee", "letmein", "New Bee", "newbee@habuma.com", true);
        spitterRepository.save(spitter);//保存，清除缓存
        spitterRepository.findAll();//进入方法，加缓存
        System.out.println(spitterRepository.count());//清除缓存，测试结束
    }
