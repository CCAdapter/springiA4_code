...
//		mongoOps.save(order, "order");//MongoTemplate的保存方法
        assertEquals(1, orderRepository.count());
//		mongoOps.getCollection("order").count();

        // Finding an order by ID
        Order foundOrder = orderRepository.findOne(savedOrder.getId());
//		mongoOps.findById(savedOrder.getId(), Order.class);
...
//		mongoOps.find(Query.query(Criteria.where("customer").is("Chuck Wagon")), Order.class);
